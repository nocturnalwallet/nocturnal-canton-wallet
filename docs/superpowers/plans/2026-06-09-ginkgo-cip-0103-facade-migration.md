...
Load the unpacked extension from `build/chrome-mv3` against a locally-running `canton-exchange-backend` (port 3003). Run through:
  ...
  7. `< BLOCKED 2026-06-10: ⚪ POST /transfer-offer/prepare → 500 Internal server error. Root cause is a pre-existing backend bug, NOT a Phase 3 regression: WalletSdkService in canton-exchange-backend uses AUTH0_AUDIENCE for all sub-clients (ledger + token + amulet), but the validator at validator-api-devnet.kairo.ag requires a different audience (VALIDATOR_AUDIENCE=https://angelhack-validator-canton-devnet, scope=validator_api). Backend log: "The supplied authentication is invalid" from validator scan-proxy. Phase 3 did not touch /transfer-offer/* or WalletSdkService — this code path was identical before and after the migration. Tracked as a separate backend follow-up; will retry once fixed and redeployed. >`
     ...
     `< UNBLOCKED 2026-06-10 (backend fix verified): canton-exchange-backend commit 410aed4 on feat/CIP-0103_migration_phase2 fixes the validator-audience mismatch — buildAuthConfig() now accepts {audience, scope} overrides and initSDK() builds a second auth config for the token+amulet sub-clients using VALIDATOR_AUDIENCE + scope=validator_api. Backend-side end-to-end smoke executed against the local docker stack: minted a JWT for kairo.dex01@gmail.com (sub=d376954e-...), POST /transfer-offer/prepare with a 0.0001 Amulet self-transfer to kairo-devnet::1220275036... → HTTP 201 with valid {preparedTransaction, preparedTransactionHash, hashingSchemeVersion: HASHING_SCHEME_VERSION_V2, commandId: transfer-offer-b43f7130-...}. Backend log shows 3 "Fetched admin token" entries at boot (one per sub-client) and ZERO "supplied authentication is invalid" errors since. This unblocks the wallet-popup-side retest of Item 7 — pending: re-run the popup transfer flow from the Ginkgo extension UI to confirm end-to-end including local signing + /transfer-offer/submit. Backend smoke proves the validator-auth layer works; the remaining wallet-side verification is a regression check that the popup's existing REST integration didn't drift in Phase 3 (it shouldn't have — Phase 3 didn't touch signing.handler.ts or api-client.ts beyond the refreshAuthTokenOnce extraction in Task 7). >`
